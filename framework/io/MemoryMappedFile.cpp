...
MemoryMappedFile::MemoryMappedFile(const std::string &filepath)
  ...
  if (size_ == 0) {
    ...
    // let's say data_ is MAP_FAILED if we can't map, but for size 0 we might
    // want "isOpen" to be true but data null? Simpler: treat empty file as "not
    // open" or just handle it. For log analysis, empty file is valid. Let's set
    // data_ to nullptr (not MAP_FAILED) and isOpen returns true? But my isOpen
    // checks != MAP_FAILED. MAP_FAILED is usually (void*)-1. Let's just handle
    // usage code to check size > 0. Actually, if size is 0, mmap fails with
    // EINVAL on some systems. Let's set fd_ to -1 if size is 0 to indicate
    // "nothing to map", OR special case:
    if (size_ == 0) {
...
bool MemoryMappedFile::isOpen() const { return data_ != MAP_FAILED; }
